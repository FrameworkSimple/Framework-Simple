...
Class Core_Helper_View
{
	...
	public static function render($file,$data=array(),$options = array())
	{
		...
		// call the beforeRender hook and if it returns false then stop the function
		if(Hook::call("beforeRender") === false) return;

		// check to see if it is an index array
		$indexed = !self::_isAssoc($data);
		...
		if($indexed)
		{
			...
			foreach($data as $snippet)
			{
				...
				$view_data = self::getContents($file_path,$snippet,$root);
				...
				Hook::call("afterRender",$view_data,$file,$id);
		...
		{
			// get the content
			$view = self::getContents($file_path,$data,$root);
			...
			if($options['layout'] && LAYOUTS)
			{
				...
				$view = self::getContents($layout_path,$options['layout_info'],$root,$view);
			}

			$id = isset($data['id'])?$data['id']:'';

			Hook::call("afterRender",$view,$file,$data);
	 ...
	 */
	public static function getContents($filename, $data=NULL,$root=NULL, $content_for_layout=NULL) {

		// if there is data
		if(is_array($data) && self::_isAssoc($data)) {
